 ...
 */
public class RemindMeAsyncQueryHandler {
    ...
    RemindMeAsyncQueryHandler() {
        mTasks = new HashMap<Integer, RemindMeAsyncQueryHandler.QueryTask>();
    }
    ...
    void cancel(final int token) {
        ...
        // If the token is not present, we need not iterate through the map at
        // all
    ...
    /**
     * @author Sharath Pandeshwar Class representing a query task
     */
    private static final class QueryTask {
         ...
         */
        private QueryTask(final Type type, final int token, final Object cookie,
                final AsyncDbQueryCallback callback) {
            mType = type;
            mToken = token;
            mCookie = cookie;
    ...
    private class QueryAsyncTask extends
        ...
        protected QueryResult doInBackground(final QueryTask... params) {
            ...
            switch (task.mType) {
                ...
                case DELETE: {
                    result.mDeleteCount = RemindMeSQLiteOpenHelper
                            .getInstance(RemindMeApplication.getStaticContext())
                ...
                case UPDATE: {
                    result.mUpdateCount = RemindMeSQLiteOpenHelper
                            .getInstance(RemindMeApplication.getStaticContext())
                            .update(task.mTableName, task.mValues, task.mSelection,
                                    task.mSelectionArgs, task.mAutoNotify);
                    break;

                }

                case QUERY: {
                    result.mCursor = RemindMeSQLiteOpenHelper
                            .getInstance(RemindMeApplication.getStaticContext())
                            .query(task.mDistinct, task.mTableName, task.mColumns, task.mSelection,
                                    task.mSelectionArgs, task.mGroupBy, task.mHaving,
                                    task.mOrderBy, task.mLimit);
                    break;
                }
            }
        ...
        protected void onPostExecute(final QueryResult result) {
            ...
            if (!task.mCancelled) {
                switch (task.mType) {
                    ...
                    case INSERT: {
                        task.mCallback.onInsertComplete(task.mToken, task.mCookie,
                                result.mInsertRowId);
                        break;
                    }

                    case DELETE: {
                        task.mCallback.onDeleteComplete(task.mToken, task.mCookie,
                                result.mDeleteCount);
                        break;
                    }

                    case UPDATE: {
                        task.mCallback.onUpdateComplete(task.mToken, task.mCookie,
                                result.mUpdateCount);
                        break;
                    }
